...
type clientUser interface {
	...
	RequestAnimeList(ctx context.Context, path string, q interface{}) ([]mal_type.UserAnime, *mal_client.Response, error)
}
...
func (s *UserService) MyInfo(ctx context.Context, opts *mal_prm.UserMyInfoRequestParameters) (*mal_type.User, *mal_client.Response, error) {
	...
	u := new(mal_type.User)
	resp, err := s.client.RequestGet(ctx, "users/@me", u, opts)
	if err != nil {
		return nil, resp, err
	}
	return u, resp, err
...
func (s *UserService) MangaList(ctx context.Context, username string, opts *mal_prm.UserMangaListRequestParameters) ([]mal_type.UserManga, *mal_client.Response, error) {
	if opts == nil {
		opts = &mal_prm.UserMangaListRequestParameters{}
	}

	list, resp, err := s.client.RequestMangaList(ctx, fmt.Sprintf("users/%s/mangalist", username), opts)
	if err != nil {
		return nil, resp, err
	}
...
func (s *UserService) AnimeList(ctx context.Context, username string, opts *mal_prm.UserAnimeListRequestParameters) ([]mal_type.UserAnime, *mal_client.Response, error) {
	if opts == nil {
		opts = &mal_prm.UserAnimeListRequestParameters{}
	}

	list, resp, err := s.client.RequestAnimeList(ctx, fmt.Sprintf("users/%s/animelist", username), opts)
	if err != nil {
		return nil, resp, err
	}
	return list, resp, nil
